...
/*
    Zac Carpenter
    4/09/2021
    Purpose: to learn more about how guard statements work.
 */
...
func createUser() -> User? {
    guard let fnField = firstNameTextField.text, let lnField = lastNameTextField.text, let ageField = ageTextField.text else {
        return nil
    }
    return User(firstName: fnField, lastName: lnField, age: ageField)
}
/*:
...
if let tempUser = user {
    print("\(tempUser.firstName) \(tempUser.lastName) is \(tempUser.age) old.")
}
